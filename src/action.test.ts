...
describe("CoderTaskAction", () => {
	...
	describe("parseGithubIssueUrl", () => {
		...
		test("throws when no issue URL provided", () => {
			...
			expect(() =>
				(action as unknown as CoderTaskAction).parseGithubIssueURL(),
			).toThrowError("Missing issue URL");
		});
		...
		test("throws for invalid URL format", () => {
			...
			expect(() =>
				(action as unknown as CoderTaskAction).parseGithubIssueURL(),
			).toThrowError("Invalid issue URL: not-a-url");
		});
	...
	describe("generateTaskUrl", () => {
		test("generates correct task URL", () => {
			...
			expect(result).toBe(`https://coder.test/tasks/testuser/${mockTask.id}`);
		});
	...
	test("sends prompt to existing task", async () => {
		...
		expect(coderClient.mockSendTaskInput).toHaveBeenCalledWith(
			mockUser.username,
			mockTask.id,
	...
	test("comments on issue", async () => {
		...
		expect(octokit.rest.issues.createComment).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				repo: "repo",
				issue_number: 123,
				body: `Task created: https://coder.test/tasks/testuser/${mockTask.id}`,
			}),
		);
	});
	...
	describe("commentOnIssue toggle", () => {
		...
		test("comments when commentOnIssue is true (explicit)", async () => {
			// Setup
			coderClient.mockGetCoderUserByGithubID.mockResolvedValue(mockUser);
			coderClient.mockGetTask.mockResolvedValue(null);
			coderClient.mockGetTemplateByOrganizationAndName.mockResolvedValue(
		...
		test("comments by default when commentOnIssue is not specified", async () => {
			// Setup
			coderClient.mockGetCoderUserByGithubID.mockResolvedValue(mockUser);
			coderClient.mockGetTask.mockResolvedValue(null);
			coderClient.mockGetTemplateByOrganizationAndName.mockResolvedValue(
			...
			const inputs = createMockInputs({
				githubUserID: 12345,
				githubIssueURL: "https://github.com/owner/repo/issues/123",
				// commentOnIssue not specified - should default to true
			});
			const action = new CoderTaskAction(
				coderClient,
				octokit as unknown as Octokit,
		...
		test("does not comment on existing task when commentOnIssue is false", async () => {
			// Setup
			coderClient.mockGetCoderUserByGithubID.mockResolvedValue(mockUser);
			coderClient.mockGetTask.mockResolvedValue(mockTask);
			coderClient.mockGetTemplateByOrganizationAndName.mockResolvedValue(
